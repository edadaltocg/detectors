...
def vit_base_timm(patch_size=16, num_classes=1000, **kwargs):
...
@timm_register_model
def dino_vitb16_ft_in1k(pretrained=True, **kwargs):
    """
    ViT-Base/16x16 pre-trained with DINO.
    Achieves 76.1% top-1 accuracy on ImageNet with k-NN classification.
    """
    default_cfg = _cfg()
    model = vit_base_timm(patch_size=16, num_classes=1000, **kwargs)
    # override timm config
    model.default_cfg = default_cfg
    model.pretrained_cfg = default_cfg
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            url="https://dl.fbaipublicfiles.com/dino/dino_vitbase16_finetune/dino_vitbase16_finetune.pth",
            map_location="cpu",
        )["model"]
        model.load_state_dict(state_dict, strict=True)
    return model
...
def dino_vitb8(pretrained=True, num_classes=0, **kwargs):
    """
    ViT-Base/8x8 pre-trained with DINO.
    Achieves 77.4% top-1 accuracy on ImageNet with k-NN classification.
    """
    model = vit_base(patch_size=8, num_classes=num_classes, **kwargs)
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
