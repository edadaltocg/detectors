        ...
        for f_ind, feat in enumerate(feat_list[:LAYERS]):
            for p_ind, p in enumerate(powers):
                ...
                for l_ind, label in enumerate(labels):
                    label = label.item()
                    Mins[label][f_ind][p_ind][: len(current_min)] = np.minimum(
                        current_min.data.cpu().numpy(), Mins[label][f_ind][p_ind][: len(current_min)]
                    )
                    Maxs[label][f_ind][p_ind][: len(current_max)] = np.maximum(
                        current_max.data.cpu().numpy(), Maxs[label][f_ind][p_ind][: len(current_max)]
                    )
        ...
        for f_ind, feat in enumerate(feat_list[:LAYERS]):
            for p_ind, p in enumerate(powers):
                ...
                for l_ind, label in enumerate(pred_labels):
                    label = label.item()
                    Eva[f_ind] += (
                        np.maximum(Mins[label][f_ind][p_ind][:dim] - g_p[l_ind], 0)
                        / np.abs(Mins[label][f_ind][p_ind][:dim] + 10**-6)
                    ).sum()
                    Eva[f_ind] += (
                        np.maximum(g_p[l_ind] - Maxs[label][f_ind][p_ind][:dim], 0)
                        / np.abs(Maxs[label][f_ind][p_ind][:dim] + 10**-6)
                    ).sum()
